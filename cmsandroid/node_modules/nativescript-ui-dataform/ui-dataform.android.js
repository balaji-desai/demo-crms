        ...
        this.groupPropertyChangedHandler = function (propertyChangeData) {
            ...
            switch (propertyChangeData.propertyName) {
                ...
                case "titleStyle":
                    ...
                    break;
                case "name":
                    var group_1 = propertyChangeData.object;
                    if (group_1.properties) {
                        for (var i = 0; i < group_1.properties.length; i++) {
                            var property = group_1.properties[i];
                            if (property.android) {
                                property.android.setGroupName(group_1.name);
                            }
                        }
                        that.get().reload();
                    }
                ...
                if (that.get().groups) {
                    for (var i = 0; i < that.get().groups.length; i++) {
                        ...
                        if (propertyGroup.name === name.toString()) {
                            var group_2 = void 0;
                            if (propertyGroup.collapsible) {
                                group_2 = new com.telerik.widget.dataform.visualization.ExpandableEditorGroup(context, name.toString());
                                that.get()._addIsExpandedChangedListener(group_2);
                                if (propertyGroup.collapsed) {
                                    group_2.setIsExpanded(false);
                                }
                            }
                            else {
                                group_2 = new com.telerik.widget.dataform.visualization.EditorGroup(context, name.toString());
                            }
                            if (propertyGroup.hidden) {
                                group_2.rootLayout().setVisibility(android.view.View.GONE);
                            }
                            if (propertyGroup.titleHidden) {
                                group_2.getHeaderContainer().setVisibility(android.view.View.GONE);
                            }
                            ...
                            that.get()._updateGroupLayout(propertyGroup, group_2);
...
var EntityProperty = /** @class */ (function (_super) {
    ...
    EntityProperty.prototype.onEditorChanged = function (oldValue, newValue) {
        ...
        if (newValue instanceof commonModule.PropertyEditor) {
            ...
            newValue.on(observable_1.Observable.propertyChangeEvent, function (propertyChangeData) {
                if (propertyChangeData.propertyName === "type") {
                    that_1.get()._onEditorTypeChanged(propertyChangeData);
                }
            });
